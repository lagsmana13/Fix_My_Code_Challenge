...
class User:
    ...
    def password(self, pwd):
        """
        Password setter:
        - Set to `None` if `pwd` is `None`
        - Set to `None` if `pwd` is not a string
    ...
    def is_valid_password(self, pwd):
        """
        Validate password:
        - Return `False` if `pwd` is `None`
        - Return `False` if `pwd` is not a string
        - Return `False` if `__password` is `None`
